...
import "package:prodajanekretnina_mobile_novi/models/pagedResult.dart";
...
class NekretnineProvider<T> with ChangeNotifier {
...
Future<PagedResult<Nekretnina>> getPaged({required int page, required int pageSize}) async {
  var url = "$_baseUrl$_endpoint?page=$page&pageSize=$pageSize";
  var uri = Uri.parse(url);
  ...
  if (isValidResponseCode(response)) {
    var data = jsonDecode(response.body);
    return PagedResult<Nekretnina>.fromJson(
      data,
      (item) => Nekretnina.fromJson(item),
    );
  } else {
    throw Exception("Greška pri dohvaćanju stranice $page");
  }
}
  ...
  Future<List<Nekretnina>> recommend(int userId) async {
  var url = "$_baseUrl$_endpoint/recommend?userId=$userId";
  var uri = Uri.parse(url);
  Map<String, String> headers = createHeaders();
  ...
  if (isValidResponseCode(response)) {
    var data = jsonDecode(response.body); // ovo je List
var nekretnine = (data as List)
    .map((item) => Nekretnina.fromJson(item))
    .toList();
return nekretnine;
